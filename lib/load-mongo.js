...
function processMovie(movie) {
    ...
    function findMovieByTitle(movie) {
        return new Promise((resolve, reject) => {
            ...
            request(options, (err, res, body) => {
                return err ? reject(err) : resolve({ id: body.results[0].id, movie_dir: movie });
            });
        });
    }

    function getMovieDataById(movieData) {
        return new Promise((resolve, reject) => {
            const options = {
                url: `https://api.themoviedb.org/3/movie/${movieData.id}?api_key=${tmdbKey}&language=en-US`,
                json: true
            };

            request(options, (err, res, body) => {
                body['movie_dir'] = movieData.movie_dir;
    ...
    function addMovieToMongo(data) {
        return new Promise((resolve, reject) => {
            const movieDir = fs.readdirSync(`${batchDir}/${data.movie_dir}`);
